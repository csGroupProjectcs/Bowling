...
TEST_F(BowlingTests, toString_compare_given_data_and_shown_data_expect_return_true)
{
    //GIVEN
    Lane lane1;
    Game game1("Name:7-|9-|X|X|6/|7-|3-|6-|7");
    lane1.addGame(game1);
    bowling.addLane(lane1);
    const static std::string data = "### Lane 1: game in progress ###\nName 102\n";
    //WHEN
    const static std::string dataFromMethod = bowling.toString();
    //THEN
...
TEST_F(BowlingTests, toString_compare_many_games_and_shown_data_expect_return_true)
{
    ...
    lane1.addGame(game3);
    bowling.addLane(lane1);
    const static std::string data = "### Lane 1: game finished ###\nName1 119\nName2 114\nName3 144\n";
    //WHEN
    const static std::string dataFromMethod = bowling.toString();
    //THEN
    ASSERT_EQ(data, dataFromMethod);
